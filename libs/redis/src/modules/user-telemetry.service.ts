...
export class UserTelemetryService {
  ...
  public async createUserRouteTelemetry(userId: string, routeId: string, data: any){
    return this.redis.getClient().hset(this.userRouteTelemetryKey(userId), routeId,JSON.stringify(data))
  }

  public async updateUserRouteTelemetry(userId: string, routeId: string, data: IUpdateUserTelemetry){
