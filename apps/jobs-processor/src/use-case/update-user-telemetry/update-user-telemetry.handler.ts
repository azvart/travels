...
export class UpdateUserTelemetryHandler {
  ...
  public async run(){
      ...
      const freshData = (await Promise.all(mapData.map(async (item) => await this.userTelemetryService.getUserRouteTelemetry(item.userId, item.routeId)))).flatMap((item) => item ? item : []);
