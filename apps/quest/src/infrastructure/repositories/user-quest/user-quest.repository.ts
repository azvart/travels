...
export class UserQuestRepository implements UserQuestAbstractRepository {
  ...
  public async attachQuestToUser(userId: string, questId: string | string[]) {
    if(Array.isArray(questId)){
      const userQuests = questId.map((id) => this.userQuestRepository.create({
        userId,
        questId: id,
      }));
    ...
    return this.userQuestRepository.save(this.userQuestRepository.create({
      userId,
      questId
    }))
  }
  ...
  public async completeQuest(userId: string, questId: string | string[]){
    if(Array.isArray(questId)){
      const updatedUserQuests = (await Promise.all(questId.map((id) => this.userQuestRepository.update({
        userId,
        questId: id
      }, {
      ...
      return Promise.all(questId.map((id) => this.userQuestRepository.findOneOrFail({
        where: {
          userId,
          questId: id
        }
